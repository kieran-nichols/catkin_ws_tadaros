...
OSAL_THREAD_FUNC simpletest(int input[6])
{
    ...
    if (ec_init(ifname))
    {
        ...
        while(inOP){
            ...
            //~ if(wkc >= expectedWKC){ // need to verify if this should go before
            if ((val->status & 0x0fff) == 0x0237 and (val2->status & 0x0fff) == 0x0237) {                
...
void extra_function(clock_t start_time) {
    ...
    while(ros::ok()){
        //~ if(itr!=0) {
            ...
            if (itr_fail > 1500) motor_listen.motor_fail = 1;
